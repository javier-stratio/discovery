...
const mapStateToProps = (state) => {
    ...
    const tables = state.app.tables
    const segments = state.app.segments.filter(s => s.table_id === table.id).slice(0, 3)
    const metrics = state.app.metrics.filter(m => m.table_id === table.id)
    const connectedTables = state.app.tables.filter(t => t.db.id === table.db.id).slice(0, 4)
    ...
    return {
        ...
        segments,
        connectedTables,
        showQB: state.curi.response.params.qb,
        scalar: state.curi.response.params.scalar,
        time: state.curi.response.params.time,
        spaces: state.app.spaces,
        edit: state.curi.response.params.edit
    }
}
...
class Table extends React.Component {
    ...
    renderViz () {
        ...
        if(time) {
            return <img src={process.env.PUBLIC_URL + '/time_series_chart.png'} style={{ width: '100%', height: '100%' }} />
        }
        return <FakeTable />
    }
    ...
    render () {
        ...
        return (
            <Box>
                ...
                {showQB && (
                    <Flex>
                        <img src={process.env.PUBLIC_URL + '/query_builder_raw_data.png'} />
