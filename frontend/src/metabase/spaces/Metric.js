...
const Badge = () =>
    <img src={process.env.PUBLIC_URL + '/verified_badge.png'} />

const mapStateToProps = (state) => {
    const metric = getMetricById(state)
    const space = getCurrentSpace(state)
    const source = state.app.tables.filter(t => t.id === metric.table_id)[0]
    const metrics = state.app.metrics.filter(m => m.table_id === metric.table_id)
    const segments = state.app.segments.filter(m => m.table_id === metric.table_id)
    return {
        metric,
        space,
        source,
        ...
        segments,
        filtered: state.curi.response.params.segmentId
    }
}
                ...
                <Flex mt={4}>
                    <Box w={showQB ? 3/3 : 2/3}>
                        <Card style={{ height: 600, overflow: 'scroll' }}>
                            <img src={process.env.PUBLIC_URL + '/time_series_chart.png'} style={{ width: '100%', height: '100%' }} />
                        </Card>
                        { !showQB && (
                            <Box>
                                { segments.length > 0 && (
